...
class ServerlessProviderAws {
  ...
  downloadFromProjectBucket(params, stage, region) {
    ...
    if (!region) region = this._S.getProject().getVariables().projectBucketRegion;
    if (!region) {
      throw new SError(`Could not find 'projectBucketRegion' variable containing project bucket's region`);
    }
