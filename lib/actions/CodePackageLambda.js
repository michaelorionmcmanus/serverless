  ...
  class Packager {
    ...
    _addEnvVarsToDist() {
      ...
      const project = _this.S.getProject(),
        Key         = ['serverless', project.getName(), _this.evt.options.stage, _this.evt.options.region, 'envVars', '.env'].join('/'),
        Bucket      = project.getVariablesObject().projectBucket;

      SUtils.sDebug(`Getting ENV Vars: ${Bucket} - ${Key}`);

      return this.S.getProvider().downloadFromProjectBucket({Bucket, Key}, _this.evt.options.stage)
        ...
        .then(reply => {
          ...
          SUtils.writeFileSync(_this.pathDist, vars);
        });
    }
